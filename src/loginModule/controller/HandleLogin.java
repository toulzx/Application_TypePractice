...
public class HandleLogin {
	...
	/**
	 * 登陆模型处理方法：
	 * 对于参数“登陆模型”，从文件中连续读取用户json数据并转换为GameModel对象；核对读取用户与登陆用户是否相同
	 * 	—— $若相同，核对密码是否正确
	 * 		—— 若密码正确，设置LoginSuccess为true; 当前用户即为此用户；
	 * 		—— 若密码错误，设置LoginSuccess为false; 设置当前用户为null。
	 * 	—— $若读取到文件结尾，仍然未查找核对成功，设置LoginSuccess为false; 设置当前用户为null。
	 * @param login 登陆模型
